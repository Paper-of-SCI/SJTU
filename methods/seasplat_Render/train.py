...
from gaussian_renderer import MediumRenderConfig, render, render_depth, render_medium, network_gui
import sys
...
_medium_field_module = _load_root_module("sjtu_medium_field", "modules/medium_field.py")
_losses_module = _load_root_module("sjtu_medium_losses", "modules/losses.py")
MediumField = _medium_field_module.MediumField
scene_medium_config = _medium_field_module.scene_medium_config
underwater_loss = _losses_module.underwater_loss


def build_medium_field(scene: Scene, opt_params) -> MediumField:
    """Create the low-capacity 3D medium field for the direct SeaSplat path."""
    camera_centers = torch.stack([cam.camera_center.detach().cpu() for cam in scene.getTrainCameras()], dim=0)
    config = scene_medium_config(
        scene.gaussians.get_xyz.detach().cpu(),
        camera_centers,
        num_samples=opt_params.medium_samples,
        hidden_dim=opt_params.medium_hidden_dim,
        density_bias=opt_params.medium_density_bias,
    )
    return MediumField(config).cuda()


def build_medium_render_config(scene: Scene, opt_params) -> MediumRenderConfig:
    """Resolve the narrow render config consumed by ``render_medium``."""
    medium_far = float(opt_params.medium_far)
    if medium_far <= 0.0:
        medium_far = float(scene.cameras_extent) * 4.0
    return MediumRenderConfig(
        medium_far=max(medium_far, 1.0e-3),
        chunk_pixels=max(int(opt_params.medium_chunk_pixels), 1),
        max_density=max(float(opt_params.medium_max_density), 0.0),
        max_optical_depth=max(float(opt_params.medium_max_optical_depth), 1.0),
    )


def save_medium_checkpoint(model_path: str, iteration: int, medium_field: MediumField | None) -> None:
    """Persist the medium sidecar checkpoint if the new formula is enabled."""
    if medium_field is None:
        return
    path = Path(model_path) / f"medium_{iteration}.pt"
...
def training(model_params, opt_params, pipe_params, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    use_medium_seathru = bool(opt_params.do_seathru and not opt_params.legacy_seathru)
    use_legacy_seathru = bool(opt_params.do_seathru and opt_params.legacy_seathru)
    medium_field = build_medium_field(scene, opt_params) if use_medium_seathru else None
    medium_render_config = build_medium_render_config(scene, opt_params) if use_medium_seathru else None
    medium_optimizer = (
        torch.optim.Adam(medium_field.parameters(), lr=opt_params.medium_lr)
        if medium_field is not None
        else None
    )

    # deep see color
    if use_legacy_seathru:
    ...
    medium_density_l1 = torch.Tensor([0.0]).squeeze().cuda()
    medium_density_smooth = torch.Tensor([0.0]).squeeze().cuda()
    medium_decor_loss = torch.Tensor([0.0]).squeeze().cuda()
    ...
    while iteration < opt_params.iterations + 1:
        ...
        seathru_active = bool(opt_params.do_seathru and iteration > opt_params.seathru_from_iter)
        medium_active = bool(use_medium_seathru and seathru_active)
        legacy_active = bool(use_legacy_seathru and seathru_active)
        ...
        # Render
        if medium_active:
            render_pkg = render_medium(viewpoint_cam, gaussians, pipe_params, medium_field, medium_render_config)
        else:
        ...
        if opt_params.learn_background and not medium_active:
            if opt_params.bg_from_bs and legacy_active:
        ...
        if opt_params.filter_depth and not medium_active:
        ...
        if medium_active:
            effective_lambda_medium = opt_params.lambda_medium if iteration > opt_params.medium_warmup_steps else 0.0
            effective_lambda_decor = opt_params.lambda_decor if iteration > opt_params.medium_warmup_steps else 0.0
            loss, medium_parts = underwater_loss(
                image.clamp(0.0, 1.0),
                gt_image,
                render_pkg["rgb_object"],
                render_pkg["rgb_medium"],
                render_pkg["metadata"]["medium_density_l1"],
                render_pkg["metadata"]["medium_density_smooth"],
                lambda_dssim=opt_params.lambda_dssim,
                lambda_medium=effective_lambda_medium,
                lambda_beta=opt_params.lambda_beta,
                lambda_decor=effective_lambda_decor,
            )
            Ll1 = medium_parts["l1"]
            l_ssim = medium_parts["ssim"]
            medium_density_l1 = medium_parts["medium_density_l1"]
            medium_density_smooth = medium_parts["medium_density_smooth"]
            medium_decor_loss = medium_parts["decor"]
            dl1 = torch.Tensor([0.0]).squeeze().cuda()
        elif legacy_active:
        ...
        if not medium_active:
            loss = (1.0 - opt_params.lambda_dssim) * Ll1 + opt_params.lambda_dssim * (1.0 - l_ssim)

        # depth weighted l1
        if opt_params.add_recon_depth_l1 and not medium_active:
            if legacy_active:
        ...
        # depth l1 loss
        if opt_params.use_depth_l1_loss and not medium_active:
        ...
        # depth smooth loss
        if opt_params.use_depth_smooth_loss and not medium_active:
        ...
        # alpha smooth loss
        if opt_params.use_alpha_smooth_loss and not medium_active:
            gt_rgb_batch = torch.unsqueeze(gt_image, dim=0)
        ...
        # gray world loss
        if opt_params.use_gw_loss and not medium_active:
        ...
        if opt_params.use_dcp_loss and not medium_active:
            gt_rgb_batch = torch.unsqueeze(gt_image, dim=0)
            if legacy_active:
        ...
        if medium_optimizer is not None:
            medium_optimizer.zero_grad()
        if opt_params.learn_background and not medium_active:
        ...
        with torch.no_grad():
            ...
            if medium_active:
                if tb_writer:
                    ...
                    tb_writer.add_scalar('train_losses/ssim', l_ssim.item(), iteration)
                    tb_writer.add_scalar('train_losses/medium_density_l1', medium_density_l1.item(), iteration)
                    tb_writer.add_scalar('train_losses/medium_density_smooth', medium_density_smooth.item(), iteration)
                    tb_writer.add_scalar('train_losses/medium_decor_loss', medium_decor_loss.item(), iteration)
                    ...
                    if iteration % 100 == 0:
                        tb_writer.add_images("medium/rgb_object", render_pkg["rgb_object"].unsqueeze(0), global_step=iteration)
                        tb_writer.add_images("medium/rgb_medium", render_pkg["rgb_medium"].unsqueeze(0), global_step=iteration)
                        tb_writer.add_images("medium/pred_image", image.unsqueeze(0), global_step=iteration)
            elif legacy_active:
                training_report(
            ...
            if iteration < opt_params.iterations:
                gaussians.optimizer.step()
                if medium_active and medium_optimizer is not None:
            ...
            if (iteration in saving_iterations):
                ...
                save_medium_checkpoint(scene.model_path, iteration, medium_field)
    ...
    if not skip_eval_train:
        ...
        with torch.no_grad():
            render_set(
                ...
                bs_model,
                at_model,
                legacy_seathru=use_legacy_seathru,
                medium_field=medium_field,
                medium_config=medium_render_config,
