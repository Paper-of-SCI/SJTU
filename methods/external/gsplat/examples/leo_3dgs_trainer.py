...
class Config:
    ...
    # 中文：已训练好的 .pt checkpoint 路径；提供后会跳过训练，只做评估/渲染。
    ckpt: Optional[List[str]] = None
    # Name of compression strategy to use
    # 中文：模型压缩方式；目前可选 "png"，None 表示不压缩。
    compression: Optional[Literal["png"]] = None
    # Render trajectory path: "interp", "ellipse", "spiral", or "raw" (use captured poses as-is)
    # 中文：视频渲染轨迹类型："interp" 插值、"ellipse" 椭圆、"spiral" 螺旋、"raw" 使用原始相机轨迹。
    render_traj_path: str = "interp"

    # Dataset backend: "colmap" or "ncore"
    # 中文：数据集后端类型；"colmap" 用普通 COLMAP 数据，"ncore" 用 NVIDIA NCore 数据。
    data_type: str = "colmap"
    # Path to the Mip-NeRF 360 dataset (colmap) or NCore v4 meta-JSON file (ncore)
    # 中文：数据路径；colmap 时是场景目录，ncore 时是 NCore v4 的 meta JSON 文件。
    data_dir: str = "data/360_v2/garden"
    # Downsample factor for the dataset
    # 中文：图像下采样倍数；数值越大图越小，训练更快但细节更少。
    data_factor: int = 4
    # Directory to save results
    # 中文：训练输出目录，ckpt、评估统计、渲染图、TensorBoard 日志都会放这里。
    result_dir: str = "results/garden"
    # Every N images there is a test image
    # 中文：测试集抽样间隔；每隔 N 张图取一张作为测试/验证图。
    test_every: int = 8
    # Random crop size for training  (experimental)
    # 中文：随机裁剪训练尺寸；None 表示用整张图，实验性功能。
    patch_size: Optional[int] = None
    # A global scaler that applies to the scene size related parameters
    # 中文：场景全局缩放因子，会影响和场景尺度相关的参数。
    global_scale: float = 1.0
    # Normalize the world space
    # 中文：是否把世界坐标归一化到更稳定的训练尺度。
    normalize_world_space: bool = True
    # Camera model
    # 中文：相机模型；普通 COLMAP 通常是 "pinhole"。
    camera_model: CameraModel = "pinhole"
    # Load EXIF exposure metadata from images (if available)
    # 中文：是否读取图片 EXIF 里的曝光信息；有曝光数据时可用于曝光/颜色处理。
    load_exposure: bool = True

    # --- NCore-specific options (only used when data_type="ncore") ---
    # 中文：NCore 专用参数，只在 data_type="ncore" 时使用。
    # Camera sensor IDs to load (auto-detected from sequence if empty)
    # 中文：要加载的 NCore 相机传感器 ID；为空时自动检测。
    ...
    # 中文：训练 batch size；学习率会按 batch size 自动缩放。
    batch_size: int = 1
    # A global factor to scale the number of training steps
    # 中文：训练步数整体缩放系数，用来快速把所有 step 类配置一起放大/缩小。
    ...
    # 中文：在这些 step 保存 .pt checkpoint。
    save_steps: List[int] = field(default_factory=lambda: [7_000, 30_000])
    # Whether to save ply file (storage size can be large)
    # 中文：是否额外保存 .ply 点云/高斯文件；文件可能很大。
    save_ply: bool = False
    # Steps to save the model as ply
    # 中文：在这些 step 保存 .ply 文件。
    ply_steps: List[int] = field(default_factory=lambda: [7_000, 30_000])
    # Whether to disable video generation during training and evaluation
    # 中文：是否关闭训练/评估过程中的轨迹视频生成。
    disable_video: bool = False

    # Initialization strategy
    # 中文：高斯初始化方式；"sfm" 表示使用 COLMAP/SfM 点云初始化。
    init_type: str = "sfm"
    # Initial number of GSs. Ignored if using sfm
    # 中文：随机初始化时的初始高斯数量；使用 sfm 时忽略。
    init_num_pts: int = 100_000
    # Initial extent of GSs as a multiple of the camera extent. Ignored if using sfm
    # 中文：随机初始化的空间范围，按相机范围的倍数计算；使用 sfm 时忽略。
    init_extent: float = 3.0
    # Degree of spherical harmonics
    # 中文：球谐颜色阶数；越高表达能力越强，但更慢、更占显存。
    sh_degree: int = 3
    # Cast SH coefficients to fp16 before feeding the SH kernel.
    # Parameters and Adam state stay fp32.
    # 中文：把送入 SH kernel 的球谐系数转成 fp16；参数和 Adam 状态仍保持 fp32。
    sh_fp16: bool = False
    # Turn on another SH degree every this steps
    # 中文：每隔多少 step 开启更高一阶 SH。
    ...
    # 中文：SSIM loss 权重；越大越强调结构相似性。
    ssim_lambda: float = 0.2

    # Near plane clipping distance
    # 中文：近裁剪面距离；比这个更近的点会被裁掉。
    near_plane: float = 0.01
    # Far plane clipping distance
    # 中文：远裁剪面距离；比这个更远的点会被裁掉。
    far_plane: float = 1e10

    # Strategy for GS densification
    # 中文：高斯 densification 策略，控制何时复制/分裂/删除高斯。
    strategy: Union[DefaultStrategy, MCMCStrategy] = field(
        default_factory=DefaultStrategy
    )
    # Use packed mode for rasterization, this leads to less memory usage but slightly slower.
    # 中文：是否使用 packed rasterization；显存更省，但速度可能略慢。
    packed: bool = False
    # Use sparse gradients for optimization. (experimental)
    # 中文：是否使用稀疏梯度优化；实验性功能。
    sparse_grad: bool = False
    # Use visible adam from Taming 3DGS. (experimental)
    # 中文：是否使用 Taming 3DGS 的 visible Adam；实验性功能。
    visible_adam: bool = False
    # Anti-aliasing in rasterization. Might slightly hurt quantitative metrics.
    # 中文：是否开启抗锯齿光栅化；视觉可能更平滑，但指标可能略降。
    antialiased: bool = False

    # Use random background for training to discourage transparency
    # 中文：训练时使用随机背景，减少模型靠透明度作弊。
    ...
    # 中文：不透明度/alpha 参数的学习率。
    opacities_lr: float = 5e-2
    # LR for orientation (quaternions)
    # 中文：高斯旋转四元数的学习率。
    quats_lr: float = 1e-3
    # LR for SH band 0 (brightness)
    # 中文：SH 第 0 阶颜色/亮度项的学习率。
    sh0_lr: float = 2.5e-3
    ...
    # 中文：不透明度正则权重，用于约束 opacity。
    opacity_reg: float = 0.0
    # Scale regularization
    # 中文：尺度正则权重，用于约束高斯 scale。
    scale_reg: float = 0.0
    ...
    # 中文：相机位姿优化正则，相当于 weight decay。
    pose_opt_reg: float = 1e-6
    # Add noise to camera extrinsics. This is only to test the camera pose optimization.
    # 中文：给相机外参加噪声，主要用于测试位姿优化是否能纠正误差。
    pose_noise: float = 0.0

    # Enable appearance optimization. (experimental)
    # 中文：是否优化每张图/每个相机的外观 embedding；实验性功能。
    app_opt: bool = False
    ...
    # 中文：bilateral grid 是否使用 fused 实现；仅 post_processing="bilateral_grid" 时有效。
    bilateral_grid_fused: bool = False
    # Shape of the bilateral grid (X, Y, W)
    # 中文：bilateral grid 的形状，分别是 X、Y、W 维度。
    bilateral_grid_shape: Tuple[int, int, int] = (16, 16, 8)
    # Enable PPISP controller
    # 中文：PPISP 是否启用 controller。
    ...
    # 中文：PPISP 是否使用 controller distillation；只在启用 PPISP controller 时有效。
    ...
    # 中文：PPISP controller 激活/预热步数；只在启用 PPISP controller 时有效。
    ...
    # 中文：计算 cc_* 指标时使用的颜色校正方法；只在启用后处理时相关。
    ...
    # 中文：评估时是否额外计算颜色校正后的指标：cc_psnr、cc_ssim、cc_lpips。
    use_color_correction_metric: bool = False

    # Enable depth loss. (experimental)
    # 中文：是否启用深度 loss；实验性功能。
    ...
    # 中文：每隔多少 step 写一次 TensorBoard 日志。
    tb_every: int = 100
    # Save training images to tensorboard
    # 中文：是否把训练渲染图也写入 TensorBoard。
    tb_save_image: bool = False

    # LPIPS 感知指标使用的 backbone；alex 更快，vgg 通常更重。
    lpips_net: Literal["vgg", "alex"] = "alex"

    # 3DGUT (uncented transform + eval 3D)
    # 中文：3DGUT，包含 unscented transform 和 3D 空间评估。
    # 中文：是否启用 unscented transform，用于畸变相机/复杂投影。
    with_ut: bool = False
    # 3DGUT：是否在 3D 空间评估 Gaussian response，通常和 with_ut 搭配使用。
