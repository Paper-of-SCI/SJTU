...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--grad-clip-norm", type=float, default=1.0, help="Global gradient norm clip; set 0 to disable.")
    parser.add_argument("--nonfinite-stop-patience", type=int, default=20, help="Stop after this many consecutive skipped non-finite steps; 0 disables stopping.")
    parser.add_argument("--gaussian-min-log-scale", type=float, default=-12.0, help="Minimum raw log scale after stability sanitize.")
    parser.add_argument("--gaussian-max-log-scale", type=float, default=5.0, help="Maximum raw log scale after stability sanitize.")
    parser.add_argument("--gaussian-max-sh", type=float, default=10.0, help="Maximum absolute SH coefficient after stability sanitize.")
    parser.add_argument("--medium-max-param", type=float, default=20.0, help="Maximum absolute medium MLP parameter after stability sanitize.")
...
def main() -> None:
    ...
    active_renderer = build_active_renderer(args, renderer, medium_field, scene)
    ...
    if medium_field is None:
        print("medium_field=disabled")
    else:
        print(
            f"medium_field=enabled samples={medium_field.num_samples} hidden_dim={args.medium_hidden_dim} "
            f"medium_lr={args.medium_lr:g} lambda_medium={args.lambda_medium:g} "
            f"lambda_beta={args.lambda_beta:g} lambda_decor={args.lambda_decor:g} "
            f"warmup={args.medium_warmup_steps}"
        )
    print(
        f"stability grad_clip_norm={args.grad_clip_norm:g} nonfinite_stop_patience={args.nonfinite_stop_patience} "
        f"log_scale=[{args.gaussian_min_log_scale:g},{args.gaussian_max_log_scale:g}] "
        f"max_sh={args.gaussian_max_sh:g} medium_max_param={args.medium_max_param:g}"
    )
    ...
    def skip_current_step(step: int, reason: str) -> bool:
        nonlocal total_skipped_steps, consecutive_skipped_steps, stop_reason
        optimizer.zero_grad(set_to_none=True)
        replaced = sanitize_training_state(args, model, medium_field, optimizer)
        total_skipped_steps += 1
        consecutive_skipped_steps += 1
        tqdm.write(
            f"数值保护 | 第 {step:06d} 步 | 跳过 optimizer.step | reason={reason} | "
            f"连续跳过={consecutive_skipped_steps} | 总跳过={total_skipped_steps} | replaced={replaced}"
        )
        patience = max(int(args.nonfinite_stop_patience), 0)
        if patience > 0 and consecutive_skipped_steps >= patience:
            stop_reason = f"连续 {consecutive_skipped_steps} 步出现非有限数值"
            return True
        return False
    ...
    for step in progress:
        ...
        render = active_renderer.render(model, camera)
        if not render_output_is_finite(render):
            if skip_current_step(step, "render_output_nonfinite"):
                break
            continue
        ...
        if not torch.isfinite(loss.detach()):
            if skip_current_step(step, "loss_nonfinite"):
                break
            continue

        # 反向传播会把图像误差传回 Gaussian 参数，然后 Adam 更新这些参数。
        loss.backward()
        trainable_params = optimizer_trainable_parameters(optimizer)
        grad_finite, grad_norm = gradients_are_finite(trainable_params)
        if not grad_finite:
            if skip_current_step(step, f"grad_nonfinite(norm={grad_norm})"):
                break
            continue
        if float(args.grad_clip_norm) > 0.0:
            torch.nn.utils.clip_grad_norm_(trainable_params, max_norm=float(args.grad_clip_norm))
        optimizer.step()
        replaced_after_step = sanitize_training_state(args, model, medium_field, optimizer)
        consecutive_skipped_steps = 0
        ...
        if step == 1 or step % args.log_every == 0:
            ...
            stability_text = f" sanitize_replaced={replaced_after_step}" if replaced_after_step > 0 else ""
            ...
            tqdm.write(
                ...
                f"{format_medium_loss_parts(parts)}"
                f"训练PSNR={psnr:.2f} | Gaussian={model.num_gaussians}{densify_text}{stability_text}"
            )
        ...
        if best_tracker is not None and (step % args.eval_every == 0 or step == args.iterations):
            ...
            eval_metrics = evaluate_cameras(model, active_renderer, test_cameras, lpips_evaluator)
    ...
    if stop_reason:
        print(f"训练提前停止：{stop_reason}")
    save_checkpoint(out_dir / "final.ply", model)
    save_medium_checkpoint(out_dir / "medium.pt", medium_field)
...
def build_medium_field(args: argparse.Namespace, scene, device: torch.device) -> MediumField | None:
    """Create the optional medium field for the ours method boundary."""
...
def build_active_renderer(args: argparse.Namespace, renderer: GaussianRenderer, medium_field: MediumField | None, scene):
    """Return either the plain renderer or the medium-aware renderer."""
    if medium_field is None:
        return renderer
...
def optimizer_trainable_parameters(optimizer: torch.optim.Optimizer) -> list[Tensor]:
    """Return the current optimizer parameters after possible densification mutations."""
    params: list[Tensor] = []
...
def tensor_is_finite(value: Tensor) -> bool:
    """Cheap scalar finite check for tensors that may be large images."""
    if not torch.is_floating_point(value) and not torch.is_complex(value):
        return True
    return bool(torch.isfinite(value.detach()).all().item())


def render_output_is_finite(render) -> bool:
    """Validate renderer outputs used by loss and densification."""
    for value in [render.image, render.alpha, render.depth, render.radii, render.means2d]:
        if isinstance(value, Tensor) and not tensor_is_finite(value):
            return False
    return True


def gradients_are_finite(params: list[Tensor]) -> tuple[bool, float]:
    """Return whether all existing gradients are finite and their global L2 norm."""
    total_sq = 0.0
    for param in params:
        grad = param.grad
        if grad is None:
        ...
        total_sq += float(grad_detached.float().square().sum().item())
...
def sanitize_training_state(
    args: argparse.Namespace,
    model: GaussianModel,
    medium_field: MediumField | None,
    optimizer: torch.optim.Optimizer | None = None,
) -> int:
    """Clamp trainable state at the training boundary and report non-finite replacements."""
    replaced = model.sanitize_parameters(
        min_log_scale=float(args.gaussian_min_log_scale),
        max_log_scale=float(args.gaussian_max_log_scale),
        max_feature_abs=float(args.gaussian_max_sh),
    )
    if medium_field is not None:
        replaced += sanitize_module_parameters(medium_field, max_abs=float(args.medium_max_param))
    if optimizer is not None:
        replaced += sanitize_optimizer_state(optimizer)
    return int(replaced)


def sanitize_module_parameters(module: torch.nn.Module, max_abs: float) -> int:
    """Keep generic module parameters finite without exposing module internals."""
    replaced = 0
    limit = max(float(max_abs), 0.0)
    with torch.no_grad():
        for param in module.parameters():
            mask = ~torch.isfinite(param)
...
def sanitize_optimizer_state(optimizer: torch.optim.Optimizer, max_abs: float = 1.0e8) -> int:
    """Keep Adam state finite so sanitized parameters are not corrupted next step."""
    replaced = 0
    limit = max(float(max_abs), 0.0)
    with torch.no_grad():
        for state in optimizer.state.values():
            for value in state.values():
                ...
                if count > 0:
                    ...
                    replaced += count
                if limit > 0.0:
                    value.clamp_(-limit, limit)
    return int(replaced)


def format_medium_loss_parts(parts: dict[str, Tensor]) -> str:
    """Return a compact medium loss log fragment when medium is enabled."""
    if "medium" not in parts:
        return ""
    return (
        f"medium={float(parts['medium'].detach()):.6f} | "
        f"decor={float(parts['decor'].detach()):.6f} | "
    )
...
def save_training_summary(
    ...
    summary = {
        ...
        "nonfinite_stop_patience": int(args.nonfinite_stop_patience),
        "gaussian_min_log_scale": float(args.gaussian_min_log_scale),
        "gaussian_max_log_scale": float(args.gaussian_max_log_scale),
        "gaussian_max_sh": float(args.gaussian_max_sh),
        "medium_max_param": float(args.medium_max_param),
        ...
        "stability_skipped_steps": int(total_skipped_steps),
