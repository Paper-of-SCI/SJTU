...
def training(model_params, opt_params, pipe_params, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    opt_params.densification_mode = normalize_densification_mode(opt_params.densification_mode)
    ...
    patch_densifier = PatchGuidedDensificationController(
        PatchGuidedDensificationConfig(
            patch_size=opt_params.patch_size,
            edge_weight=opt_params.patch_edge_weight,
            detail_lambda=opt_params.patch_detail_lambda,
            clone_jitter_scale=opt_params.clone_jitter_scale,
        )
    )
    ...
    while iteration < opt_params.iterations + 1:
        ...
        if opt_params.do_seathru and iteration > opt_params.seathru_from_iter:
            densification_image = underwater_image.squeeze(0)
        else:
            densification_image = image
        ...
        with torch.no_grad():
            ...
            if iteration < opt_params.densify_until_iter:
                densification_active = iteration < opt_params.freeze_gs_from_iter or iteration >= opt_params.unfreeze_gs_from_iter
                if densification_active:
                    grad_threshold = opt_params.densify_grad_threshold
                    if iteration >= opt_params.freeze_gs_from_iter and iteration >= opt_params.unfreeze_gs_from_iter:
                        grad_threshold = opt_params.scale_grad_threshold * opt_params.densify_grad_threshold
                    size_threshold = 20 if iteration > opt_params.opacity_reset_interval else None

                    # Keep track of max radii in image-space for pruning
                    if opt_params.densification_mode == "patch_guided":
                        densify_stats = patch_densifier.update(
                            gaussians=gaussians,
                            viewpoint_camera=viewpoint_cam,
                            rendered_image=densification_image,
                            gt_image=gt_image,
                            viewspace_point_tensor=viewspace_point_tensor,
                            visibility_filter=visibility_filter,
                            radii=radii,
                            iteration=iteration,
                            densification_interval=opt_params.densification_interval,
                            densify_from_iter=opt_params.densify_from_iter,
                            grad_threshold=grad_threshold,
                            min_opacity=0.005,
                            scene_extent=scene.cameras_extent,
                            max_screen_size=size_threshold,
                        )
                        if densify_stats.densified:
                            print(
                                f"[{iteration}] patch densify "
                                f"clone={densify_stats.cloned} split={densify_stats.split} prune={densify_stats.pruned} "
                                f"high_grad={densify_stats.high_grad} adjusted_grad_max={densify_stats.grad_max:.2e} "
                                f"detail_max={densify_stats.patch_detail_max:.3f} total={densify_stats.total}"
                            )
                    else:
                        gaussians.max_radii2D[visibility_filter] = torch.max(gaussians.max_radii2D[visibility_filter], radii[visibility_filter])
                        gaussians.add_densification_stats(viewspace_point_tensor, visibility_filter)

                        if opt_params.densification_interval > 0 and iteration % opt_params.densification_interval == 0:
                    ...
                    opacity_reset_due = opt_params.opacity_reset_interval > 0 and iteration % opt_params.opacity_reset_interval == 0
                    if opacity_reset_due or (model_params.white_background and iteration == opt_params.densify_from_iter):
